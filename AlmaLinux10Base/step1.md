# Step 1 - Update and setup basic tools
...
Before running this section check if there is already a swap file by using the `free` command
...
## Install Common Repositories and update
...
```
cat >>/usr/bin/setup-static-ip <<EOL
#!/usr/bin/env bash
...
chmod +x /usr/bin/setup-static-ip
```

## Set the servers static IP if needed

This is only really relevant to local development virtual machines, if this is a remotely hosted server or you are happy with the ip being set by DHCP then skip this section

```
# Setting a single IP address
setup-static-ip {your-desired-ip}

# Assigning multiple IP addresses
setup-static-ip {your-desired-ip-1} {your-desired-ip-2} {your-desired-ip-3}

# Performing a dry run
setup-static-ip --dry-run {your-desired-ip}
```

## Reboot the server

This is always wise after a system update because the updated kernal will only load in on reboot
